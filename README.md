...
<tr><td>✅</td><td><b>Cross‑chain pay‑in (CCTP V2)</b> — a buyer pays a Klaro invoice from <b>Base Sepolia</b>; the operator daemon completes Circle's burn‑and‑mint so the vendor receives native USDC on Arc. <b>Live testnet, proven on‑chain end to end</b> (real burn + Iris attestation + Arc mint). The buyer checkout enables via <code>CCTP_PAYIN_ENABLED</code> wherever the operator daemon runs to settle the mint. Circle <b>Gateway</b> routing remains pending.</td></tr>
