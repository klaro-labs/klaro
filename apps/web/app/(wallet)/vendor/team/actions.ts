...
import { z } from "zod";
import { type TeamRole } from "@/lib/mockData";
import * as teamRepo from "@/lib/repo/team";
...
async function _resolveTeammate(
  ...
  const team = await teamRepo.listTeam(vendorId);
...
export async function inviteTeammateAction(formData: FormData): Promise<void> {
  ...
  await teamRepo.inviteTeammate({ vendorId, email, role });
...
export async function changeRoleAction(
  ...
  await teamRepo.changeRole(id, parsed);
  revalidatePath("/vendor/team");
}
...
export async function removeTeammateAction(id: string): Promise<void> {
  ...
  await teamRepo.removeTeammate(id);
  revalidatePath("/vendor/team");
}
