// GOAL buyer-pay leg (node-runnable copy of pb-pay.ts; BASE env-overridable).
// Drives the REAL public /i/[id] pay path with an injected buyer wallet:
// EIP-712 acceptance sign → approve → acceptAndPay. Key stays in Node.
// Usage: KLARO_E2E_BASE_URL=https://www.myklaro.app node e2e/goal-pay.mjs <invoiceId>
...
const BASE = process.env.KLARO_E2E_BASE_URL || "http://127.0.0.1:3100";
const RPC = local.NEXT_PUBLIC_ARC_TESTNET_RPC_URL || "https://rpc.testnet.arc.network";
const CHAIN_ID = 5_042_002;
const INVOICE = process.argv[2];
if (!INVOICE) { console.error("usage: goal-pay.mjs <invoiceId>"); process.exit(2); }
const ESCROW = local.NEXT_PUBLIC_INVOICE_ESCROW_ADDRESS;
const shots = path.resolve("e2e/.goal-pay-shots");
...
log("buyer (injected account):", buyer.address, "| base:", BASE);
...
page.on("console", (m) => { const t = m.text(); if (/error|insufficient|revert/i.test(t) && !/reown|allowlist|403/i.test(t)) log("page:", t.slice(0, 160)); });
...
for (let i = 0; i < cnt; i++) {
  ...
  if (await b.isVisible().catch(() => false)) { log("clicking Pay (visible button", i + ")"); await b.click().catch(() => {}); clicked = true; break; }
}
...
let ok = false, finalStatus = -1;
for (let i = 0; i < 40; i++) {
  await page.waitForTimeout(1500);
  const o = await pub.readContract({ address: ESCROW, abi: ESC_ABI, functionName: "invoices", args: [INVOICE] }).catch(() => null);
  if (o && o[5].toLowerCase() === buyer.address.toLowerCase()) { ok = true; finalStatus = Number(o[9]); log("PAID on-chain: status", finalStatus, "acceptedBy", o[5]); break; }
}
...
await browser.close();
console.log("PAY_OK=" + ok + " status=" + finalStatus);
process.exit(ok ? 0 : 1);
